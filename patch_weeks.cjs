...
const regexBuckets = /\/\/ Set up 8-week buckets[\s\S]*?for \(let w = 1; w <= 8; w\+\) \{\s*weeklyBuckets\[w\] = \{ volume: 0, plannedVolume: 0, sessions: \[\] \};\s*\}/m;

const replacementBuckets = `
...
code = code.replace(regexBuckets, replacementBuckets);
